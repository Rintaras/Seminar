...
DELAY=${2:-0}         # 遅延（ms）
LOSS=${3:-0}          # パケット損失率（%）
BANDWIDTH=${4:-"0"}   # 帯域幅制限（例: 1mbit, 10mbit, 100mbit）
...
# 新しい設定を適用
if [ "$DELAY" -gt 0 ] || [ "$LOSS" != "0" ] || [ "$BANDWIDTH" != "0" ]; then
    if [ "$BANDWIDTH" != "0" ]; then
        # 帯域幅制限がある場合
        tc qdisc add dev $INTERFACE root handle 1: tbf rate ${BANDWIDTH} burst 32kbit latency 400ms
